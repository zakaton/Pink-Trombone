...
function setupNode(audioNode) {
  ...
  Object.defineProperty(audioNode, "constrictions", {
    get: function () {
      return this._constrictions.filter(
        (constriction) => constriction._isEnabled
      );
    },
  });
...
function assignAudioParam(audioNode, audioParam, paramName) {
  if (paramName.includes("constriction")) {
    ...
    constriction[paramName.includes("index") ? "index" : "diameter"] =
      audioParam;

    audioNode.constrictions[constrictionIndex] = constriction;
  } else if (paramName.includes("vibrato")) {
    audioNode.vibrato[paramName.replace("vibrato", "").toLowerCase()] =
      audioParam;
  } else if (paramName.includes("tongue")) {
    audioNode.tongue[paramName.replace("tongue", "").toLowerCase()] =
      audioParam;
  } else {
...
if (window.AudioWorklet !== undefined) {
  class PinkTromboneNode extends AudioWorkletNode {
    constructor(audioContext) {
      ...
      this.channelSplitter = audioContext.createChannelSplitter(channelCount);

      this.parameters.forEach((audioParam, paramName) => {
        assignAudioParam(this, audioParam, paramName);
      });

      this.connect(this.channelSplitter);
  ...
  window.AudioContext.prototype.createPinkTromboneNode = function () {
    const pinkTromboneNode = this.createScriptProcessor(
      Math.pow(2, 11),
      ParameterDescriptors.length,
      1
    );
    ...
    pinkTromboneNode.channelMerger = this.createChannelMerger(
      ParameterDescriptors.length
    );
    ...
    ParameterDescriptors.forEach((parameterDescriptor, index) => {
      ...
      audioParam.automationRate =
        parameterDescriptor.automationRate || "a-rate";
    ...
    pinkTromboneNode._getParameterChannels = function (inputBuffer) {
      ...
      for (
        ...
        parameterChannels[this.channelMerger.channels[channelIndex]] =
    ...
    pinkTromboneNode._getParameterSamples = function (
      parameterChannels,
      sampleIndex
    ) {
      ...
      for (
        let channelIndex = 0;
        channelIndex < parameterNames.length;
        channelIndex++
      ) {
        const parameterName = parameterNames[channelIndex];

        if (!parameterName.includes("constriction"))
          parameterSamples[parameterName] =
            parameterChannels[parameterName][sampleIndex];
      }
    ...
    pinkTromboneNode._getConstrictions = function (parameterChannels) {
      ...
      for (
        let constrictionIndex = 0;
        constrictionIndex < this._constrictions.length;
        constrictionIndex++
      ) {
        ...
        if (_constriction._isEnabled) {
          const constriction = {
            index:
              parameterChannels[
                "constriction" + constrictionIndex + "index"
              ][0],
            diameter:
              parameterChannels[
                "constriction" + constrictionIndex + "diameter"
              ][0],
          };
    ...
    pinkTromboneNode.onaudioprocess = function (event) {
      ...
      for (
        ...
        const parameterSamples = this._getParameterSamples(
          parameterChannels,
          sampleIndex
        );
        const bufferLength = outputChannel.length;
        const seconds =
          event.playbackTime + sampleIndex / event.inputBuffer.sampleRate;
      ...
      this.processor.update(
        event.playbackTime +
          outputChannel.length / event.inputBuffer.sampleRate,
        constrictions
      );
    };
