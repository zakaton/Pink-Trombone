...
class Tract {
  ...
  process(parameterSamples, sampleIndex, bufferLength, seconds) {
    ...
    var outputSample = 0;
    outputSample += this._processLips(
      parameterSamples,
      bufferInterpolation,
      updateAmplitudes
    );
    outputSample += this._processNose(
      parameterSamples,
      bufferInterpolation,
      updateAmplitudes
    );
      ...
      for (let index = -1; index < constrictions.length; index++) {
        ...
        if (
          ...
          if (newTractDiameter < 3) {
            ...
            else if (normalizedIndex >= upperIndex)
              tractIndexRange = upperIndexRange;
            else
              tractIndexRange =
                lowerIndexRange -
                (upperIndexRange * (normalizedIndex - lowerIndex)) / indexRange;
