...
class Tract {
  constructor(length = 44) {
    ...
    this.tongue = {
      ...
      set index(newValue) {
        const diameterInterpolation = this.range.diameter.interpolation(
          this.diameter
        );
        ...
        const straightenedInterpolation =
          ...
          0.2 *
            (Math.pow(invertedDiameterInterpolation, 2) -
              invertedDiameterInterpolation);
        const centerOffset = this.range.index.centerOffset(
          straightenedInterpolation
        );
        ...
        this._index = true
          ...
          : Math.clamp(
              newValue,
              this.range.index.minValue,
              this.range.index.maxValue
            );
      },
    };
    ...
    for (let index = 0; index < this.length; index++) {
      ...
      if (index < (7 * this.length) / 44 - 0.5) value = 0.6;
      else if (index < (12 * this.length) / 44) value = 1.1;
      else value = 1.5;
  ...
  process(parameterSamples, sampleIndex, bufferLength, seconds) {
    ...
    var outputSample = 0;
    const lips = this._processLips(
      parameterSamples,
      bufferInterpolation,
      updateAmplitudes
    );
    const nose = this._processNose(
      parameterSamples,
      bufferInterpolation,
      updateAmplitudes
    );
    outputSample += nose + lips;

    if (isNaN(outputSample)) this.reset();

    return { outputSample, nose, lips };
  }
  ...
  _processLips(parameterSamples, bufferInterpolation, updateAmplitudes) {
    this.right.junction[0] =
      this.left[0] * this.glottis.reflection + parameterSamples.glottis;
    ...
    this.left.junction[this.nose.start] =
      ...
      (leftInterpolation + 1) *
        (this.nose.left[0] + this.left[this.nose.start]);
    ...
    this.right.junction[this.nose.start] =
      ...
      (rightInterpolation + 1) *
        (this.nose.left[0] + this.right[this.nose.start - 1]);
    ...
    this.nose.right.junction[0] =
      ...
      (noseInterpolation + 1) *
        (this.left[this.nose.start] + this.right[this.nose.start - 1]);
    ...
    for (let index = 0; index < this.length; index++) {
      ...
      if (updateAmplitudes) {
        ...
        this.amplitude.max[index] =
          sum > this.amplitude.max[index]
            ? sum
  ...
  _processNose(parameterSamples, bufferInterpolation, updateAmplitudes) {
    ...
    for (let index = 1; index < this.nose.length; index++) {
      const offset =
        this.nose.reflection[index] *
        (this.nose.left[index] + this.nose.right[index - 1]);
    ...
    for (let index = 0; index < this.nose.length; index++) {
      this.nose.left[index] =
        this.nose.left.junction[index + 1] * this.nose.fade;
      ...
      if (updateAmplitudes) {
        ...
        this.nose.amplitude.max[index] =
          sum > this.nose.amplitude.max[index]
            ? sum
  ...
  _updateDiameterRest() {
    for (let index = this.blade.start; index < this.lip.start; index++) {
      const interpolation =
        (this.tongue.index - index) / (this.tip.start - this.blade.start);
  ...
  _updateConstrictions(constrictions) {
    ...
    const maxIndex = Math.max(
      this.previousConstrictions.length,
      constrictions.length
    );
    for (
      let constrictionIndex = 0,
        A = constrictions[0],
        B = this.previousConstrictions[0];
      !update && constrictionIndex < maxIndex;
      constrictionIndex++,
        A = constrictions[constrictionIndex],
        B = this.previousConstrictions[constrictionIndex]
    ) {
      update =
    ...
    if (update) {
      ...
      for (let index = -1; index < constrictions.length; index++) {
        ...
        if (
          constriction.index > this.nose.start &&
          constriction.diameter < -this.nose.offset
        )
        ...
        if (
          ...
          if (newTractDiameter < 3) {
            ...
            if (constriction.index < 25) tractIndexRange = 10;
            else if (constriction.index >= this.tip.start) tractIndexRange = 5;
            else
              tractIndexRange =
                10 - (5 * (constriction.index - 25)) / (this.tip.start - 25);
            ...
            for (
              ...
              const tractIndexOffset =
                Math.abs(tractIndex - constriction.index) - 0.5; // relpos
              ...
              if (tractIndexOffset <= 0) tractDiameterScalar = 0;
                ...
                tractDiameterScalar =
                  0.5 *
                  (1 -
                    Math.cos((Math.PI * tractIndexOffset) / tractIndexRange));

              const tractDiameterDifference =
                this.diameter[tractIndex] - newTractDiameter;
              if (tractDiameterDifference > 0) {
                this.diameter[tractIndex] =
                  newTractDiameter +
                  tractDiameterDifference * tractDiameterScalar;
              }
            }
          }
        }
    ...
    if (this.nose.amplitude[0] < 0.05) {
      if (
        ...
        this.transients.push(
          new Transient(this.transients.obstruction.new, seconds)
        );
  ...
  _updateReflection() {
    ...
    this.left.reflection.new =
      (2 * this.amplitude[this.nose.start] - sum) / sum;

    this.right.reflection.value = this.right.reflection.new;
    this.right.reflection.new =
      (2 * this.amplitude[this.nose.start + 1] - sum) / sum;
