...
class PinkTromboneWorkletProcessor extends AudioWorkletProcessor {
    ...
    this.port.onmessage = (event) => {
      switch (event.data.name) {
        ...
        case "enabledConstrictionIndices":
          event.data.enabledConstrictionIndices =
            this.enabledConstrictionIndices;
          this.port.postMessage(event.data);
          break;
  ...
  _getParameterSamples(parameters, sampleIndex) {
    ...
    for (
      ...
      const parameterDescriptor =
        this.constructor.parameterDescriptors[parameterDescriptorIndex];
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let outputIndex = 0; outputIndex < outputs.length; outputIndex++) {
      for (let channelIndex = 0; channelIndex < 1; channelIndex++) {
        for (
          let sampleIndex = 0;
          sampleIndex < outputs[outputIndex][channelIndex].length;
          sampleIndex++
        ) {
          ...
          const { outputSample, voice, nose, lips, noise, glottisSample } =
          ...
          outputs[outputIndex][1][sampleIndex] = noise;
          outputs[outputIndex][2][sampleIndex] = voice;
          outputs[outputIndex][3][sampleIndex] = glottisSample;
          outputs[outputIndex][4][sampleIndex] = nose;
          outputs[outputIndex][5][sampleIndex] = lips;
        }
      }
    }

    this.processor.update(
      currentTime + outputs[0][0].length / sampleRate,
      constrictions
    );
...
registerProcessor(
  "pink-trombone-worklet-processor",
